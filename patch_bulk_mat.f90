...
! revised from patch_bulk_lat.f90

! based on the function of patch_bulk_lat.f90, as stated below

! start----------------------------------------------------------------------------------------------------

! | ~/compile_kai/get_epl.x --> yields the ion polarization tensor chi(i,j,iatom)

! | ~/compile_patchE/patchE.x --> yields the rotation matrix op(i,j,iatom)

! | this program is used to check

! | Is op can fulfil the transformation of chi for each atom ?

! | that is to say, we want to check

! | Given that by running self-patchE, we obtain OUT.atom_motif.list, which gives 

! | motif_id(iatom) op(i,j,iatom) for iatom=1,natom

! | We can check for each iatom, that if 

! | op * chi * inv(op)  == chi_prime(imotif)

! | is a good definition for finding motif's effective ion polarization tensor ?

! | Hence, the output of the program should be the 

! | first, report each motif is used how many times in this self-patch operation, say, num_used(imotif)

! | off-course, sum of [ num_used(imotif) for {imotif = 1, num_motif} ] =  natom_patch

! | second, chi_prime(iatom) = op(iatom) * chi(iatom) * inv(op(iatom)) 

! | if motif_id(iatom) == motif_id(jatom) then

! | compare if chi_prime(iatom) == chi_prime(jatom)

! | or, just format print them out

! end-of---------------------------------------------------------------------------------------------------

! we further calculate the M-matrix, which satisfy that

! P_{:,l} = \int r(:) * < M * D, E_l > dr^3

! where P_{:,l} is the chi_prime(iatom) , and D is the motif dens corresponding to iatom

! and E_l is the unit electric field in motif coordinate system, l =1,3

! expand above equation as, using E_l = delta_l vector, so that

! P_{x,l} = S x * (M_{l,1} * D1 + M_{l,2} * D2 + M_{l,3} * D3) dr^3
! P_{y,l} = S y * (M_{l,1} * D1 + M_{l,2} * D2 + M_{l,3} * D3) dr^3
! P_{z,l} = S z * (M_{l,1} * D1 + M_{l,2} * D2 + M_{l,3} * D3) dr^3

! where "S" stands for the integeral sign.

! Denotes

! <ri,Dj> = S ri * Dj dr^3 , where r1=x, r2=y, r3=z

! Hence, the equations above are denoted as

! / P_{x,l} \    [ < x,D1 >   <x,D2>   <x,D3> ]    / M_{l,1} \
! | P_{y,l} | =  [ < y,D1 >   <y,D2>   <y,D3> ]  * | M_{l,2} |
! \ P_{z,l] /    [ < z,D1 >   <z,D2>   <z,D3> ]    \ M_{l,3} /
...
!c      parameter (m=80,rad_box=8.d0)
      ...
      real*8, allocatable, dimension(:,:,:) ::vr,rho0,vr_ext,rho_in,rho_out,vr0
      ...
      real*8,allocatable,dimension(:,:,:) :: dipmot,matset,matlis,diplat
      ...
      character(len=256) :: line_in,file_ion_ptensor
      integer*4 :: ifag,strcmp
      integer*4,allocatable,dimension(:) :: replis
...
!      read(11,*)
      ...
      do ii=1,num_Emotif
      open(12,file="MISSING_MOTIFS/E"//file_Emotif(ii),form="unformatted")
      rewind(12)
      ...
      do ia=1,num_neighM(ii)
       read(12) iat_neighM(ia,ii),x_neighM(1,ia,ii),x_neighM(2,ia,ii),x_neighM(3,ia,ii)
      enddo
...
!cccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      read(10,*) iiatom(i),x(1,i),x(2,i),x(3,i),im1,im2,im3,ipatch_flag(i)
      endif
...
!!!!! summ up the motif with the weight they are used in the list
      ...
      allocate(diplat(3,3,natom_patch))
      allocate(matset(3,3,natom_patch))
      dipmot=0.d0
      diplat=0.d0
      matset=0.d0
      num=0
...
! filling in the dipmot(3,3,natom_patch)
      file_ion_ptensor="log.get_epl"
      if(natom.ne.natom_patch) then
      write(6,*) "assume natom_patch .eq. natom, stop"
      stop
      end if
      open(11,file=trim(adjustl(file_ion_ptensor)))
      rewind(11)
      ifag=-1
      strcmp=-1
      iatom=0
      do 
      read(11,'(A)',END=101) line_in
      strcmp=index(line_in,"CHI ingredient collecting finish")
      if(strcmp.gt.0) ifag=1
      if(ifag.gt.0) then
      !write(6,*) line_in
              iatom=iatom+1
              if(iatom.gt.natom_patch) exit
               read(11,*) 
               read(11,*) diplat(1:3,1,iatom)
               read(11,*) diplat(1:3,2,iatom)
               read(11,*) diplat(1:3,3,iatom)
      end if
      end do
      close(11) 
101 write(6,*) "reach the end of ",trim(adjustl(file_ion_ptensor))
...
!!!!  emotif box volume
      ...
      !*(vole/volc)
      vol_fac=-1.d0*vole/(2*mb*0.529177d0)**3
...
!*    however, only limited EMOTIF will be stored, thus op is necessary      
      ...
      mat_tmp=matmul(op,diplat(1:3,1:3,iatom))
      mat_tmp=matmul(mat_tmp,oq)
      diplat(1:3,1:3,iatom)=mat_tmp  ! P_{1:3,1:3}

! obtain integral of r * dens_mEM on the EMOTIF box
      do jchi=1,3
        do ixyz=-mb,mb-1
      ...
      call get_ALI(dipmot(1:3,1:3,iatom),oq)
      oq=TRANSPOSE(oq)
      oq=matmul(oq,diplat(1:3,1:3,iatom))
      matset(1:3,1:3,iatom)=TRANSPOSE(oq)

      ! sum atom
      ! dipall(1:3,1:3)=dipall(1:3,1:3)+dipmot(1:3,1:3,iatom)

!                                                                 !
      write(6,"(A23,I4,A21,I4)") "--matrix-from--atom--",iatom,"---for---motif---",ind_motif0(iatom)
      write(6,"(3F16.9)") matset(:,1,iatom)
      write(6,"(3F16.9)") matset(:,2,iatom)
      write(6,"(3F16.9)") matset(:,3,iatom)
...
!      write(6,"(A16,A16,F16.9)") "------ε(∞)------","---trace-avg-->",mean
!      write(6,"(3F16.9)") dipall(:,1)
!      write(6,"(3F16.9)") dipall(:,2)
!      write(6,"(3F16.9)") dipall(:,3)

      write(6,*) "finish iatom, start to write MOTIF_matrix_factor"
      allocate(matlis(3,3,num_Emotif))
      allocate(replis(num_Emotif))
      matlis=0.d0
      replis=0
      do iatom=1,natom_patch
         replis(ind_motif0(iatom))=replis(ind_motif0(iatom))+1
         matlis(1:3,1:3,ind_motif0(iatom))=matlis(1:3,1:3,ind_motif0(iatom))+matset(1:3,1:3,iatom) 
      end do 
      do ii=1,num_Emotif 
      matlis(1:3,1:3,ii)=matlis(1:3,1:3,ii)/(1.d0*replis(ii))
      end do
      ! maybe here need residue check, i.e., take this mean is reasonable
      
      open(11,file="MOTIF_matrix_factor")
      rewind(11)
      write(11,'(I4)') num_Emotif
      do ii=1,num_Emotif
      write(11,*) ind_config(ii),ind_motif0(ii)
      write(11,*) trim(file_Emotif(ii))
      write(11,"(3F16.9)") matlis(:,1,ii)
      write(11,"(3F16.9)") matlis(:,2,ii)
      write(11,"(3F16.9)") matlis(:,3,ii)     
      enddo
      close(11)
