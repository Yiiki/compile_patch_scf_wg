...
c      parameter (m=80,rad_box=8.d0)
     ...
     &    rho_in,rho_out,vr0
      ...
      real*8, allocatable, dimension(:,:,:) :: dens,dens_tmp,matset,d1,
     & d2,d3
...
!     new: we need a matrix factor
     ...
     &      x_neighM(3,ia,ii)
      ...
      if(inode.eq.1) then
!      read(12) d1(-m:m,-m:m,-m:m)
!      read(12) d2(-m:m,-m:m,-m:m)
!      read(12) d3(-m:m,-m:m,-m:m)
...
5001  continue
      ...
      if(inode.eq.1) then
      nnodes_tmp=1
      open(13,file="dvh_patch.SCF",form="unformatted")
      rewind(13)
      ...
      do iread=1,nnodes_tmp
         do ii=1,nr_n
            ...
            j=(jj-1-(i-1)*n2*n3)/n3+1
            k=jj-(i-1)*n2*n3-(j-1)*n3
            vr_tmp(ii)=vr(i,j,k)-vr_ext(i,j,k)
         enddo
         write(13) vr_tmp
      enddo
      close(13)
