...
!      read(11,*)
      ...
      rewind(11)
      read(11,*) m,rad_box
      read(11,*) AL_mbox(1,1),AL_mbox(2,1),AL_mbox(3,1)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      open(13,file="VR.EXT",form="unformatted",
     $     status='old',action='read',iostat=ierr)
      if(ierr.ne.0) then
         write(6,*) "file ** VR.EXT **** does not exist, stop"
         call mpi_abort(MPI_COMM_WORLD,ierr)
      endif
      rewind(13)
...
! so let we try to disable it
!!!!       if(iter.eq.1) then
!!!!       call orientation_test(iat_neigh(1,iatom),iat_neighM(2,itype),
!!!!     &  dxyz_indZ(1,1,iatom),dxyz_motif(1,1,itype),
!!!!     &  num_neighM(itype)-1,num_neigh_All(iatom),op,iatom,itype)
!!!!       endif
!!!!       
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
ccccccc  now, input the electric field part, and add that into motif_tmp (the base part)
ccccccc  First, convert the Efieldtmp(3) in the x,y,z coord of the AL, into 
ccccccc  the rotated mbox x,y,z directions
...
ccccccccccccccccccccccccccccccccccccccccccccccccC
ccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine orientation_test(iat_neigh,iat_neighM,
     &  dxyz_indZ,dxyz_motif,num_neighM,num_neigh,op,iatom,itype)

ccccc This code finds a op, to rotate the dxyz_motif into dxyz_indz.
ccccc But it does not optimize the many possible rotations. It is kind
ccccc of bad. Besides, it does not allow cases where there is some
ccccc differences. It uses a linear transformation.

      implicit double precision (a-h,o-z)

      integer iat_neigh(50),iat_neighM(50)
      real*8 dxyz_indZ(3,50),dxyz_motif(3,50)
      real*8 dxyz(3)
      real*8 op(3,3)
      ...
      do j=1,3
      dxyz(j)=op(j,1)*dxyz_indZ(1,i)+op(j,2)*dxyz_indZ(2,i)+
     &  op(j,3)*dxyz_indZ(3,i)
!!!! This has shown the op (and its convention) is correct !
      ...
      dmin=1000.d0
      do i2=1,num_neighM
      dd=(dxyz(1)-dxyz_motif(1,i2))**2+
     & (dxyz(2)-dxyz_motif(2,i2))**2+(dxyz(3)-dxyz_motif(3,i2))**2
      if(dd.lt.dmin) dmin=dd
      enddo

      diff=diff+dsqrt(dmin)
      enddo

      write(6,*) "iatom,imotif",iatom,itype
      if(num_neigh.gt.0) then
      write(6,*) "orientation diff=",num_neigh,diff/num_neigh
      else
      write(6,*) "orientation diff=",num_neigh,diff
      endif
      write(6,*) "dxyz_indZ",num_neigh
      do i=1,num_neigh
      write(6,"(3(E15.6,1x))") dxyz_indZ(1,i),dxyz_indZ(2,i),
     &      dxyz_indZ(3,i)
      enddo
      write(6,*) "dxyz_motif",num_neighM
      do i=1,num_neighM
      write(6,"(3(E15.6,1x))") dxyz_motif(1,i),dxyz_motif(2,i),
     &      dxyz_motif(3,i)
      enddo
      write(6,*) "op"
      write(6,"(3(E13.5,1x))") op(1,1),op(2,1),op(3,1)
      write(6,"(3(E13.5,1x))") op(1,2),op(2,2),op(3,2)
      write(6,"(3(E13.5,1x))") op(1,3),op(2,3),op(3,3)
      write(6,*) "-------------------------"


      do j1=1,3
      do j2=1,3
      sum=op(1,j1)*op(1,j2)+op(2,j1)*op(2,j2)+op(3,j1)*op(3,j2)
      sum0=0.d0
      if(j1.eq.j2) sum0=1.d0
      if(dabs(sum-sum0).gt.1.D-5) then
      write(6,*) "transformation not orthonormal, stop",j1,j2,sum
      write(6,*) "op"
      write(6,*) op(:,1)
      write(6,*) op(:,2)
      write(6,*) op(:,3)
